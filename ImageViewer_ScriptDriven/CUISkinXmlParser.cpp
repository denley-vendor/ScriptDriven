...
const char * kNode_Script		= "Script";
...
const char * kAttr_Source		= "source";
...
NODE_TYPE CUISkinXmlParser::GetNodeTypeNumber()
{
	...
	else if (strType.Compare(kNode_Script) == 0)
	{
		return NT_Script;
	}
	else
...
CString CUISkinXmlParser::GetID()
{
	return GetAttribute(kAttr_ID);
}

CString CUISkinXmlParser::GetSource()
{
	return GetAttribute(kAttr_Source);
}
