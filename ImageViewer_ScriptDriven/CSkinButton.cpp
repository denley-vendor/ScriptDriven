...
CSkinButton::CSkinButton() 
	...
	, mCaptureRegion(0)
{
	mStateCount = 4; // Normally, a botton has 4 status.
...
/*	CUISkinXmlParser* pParser = CUISkinXmlParser::GetInstance();
	if (pParser->FindButton(mTagName))
	{
		mSkinFileName = pParser->GetSkin();
		pParser->GetPosition(mSkinXPos, mSkinYPos, mSkinWidth, mSkinHeight);
	}*/
}
