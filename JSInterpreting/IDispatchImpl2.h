...
#ifndef __H_IDispatchImpl2__
#define __H_IDispatchImpl2__

#include "CHQException.h"

template<class DERIVED_CLASS, class BASE_ITF, const IID* BASE_IID>
class __declspec(novtable) IDispatchImpl2 : public BASE_ITF
{
protected:
	long			m_cRef;
	ITypeInfo*		m_pti;
...
public:
	IDispatchImpl2() : m_pti(0), m_ptl(0), m_cRef(1) {};

	virtual ~IDispatchImpl2() 
	{
		if (m_pti)
		{
			m_pti->Release();
	...
	// --- IJSObject methods --- 
	STDMETHODIMP get__internalValue(VARIANT* value)
	{
		if (value)
		{
			value->vt = VT_BSTR;
	...
	STDMETHODIMP toLocaleString(BSTR* value)
	{
		// Defaule: no localization.
